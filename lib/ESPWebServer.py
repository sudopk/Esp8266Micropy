...
def handleClient():
    ...
    if res:  # There's a new client connection
        ...
        socket.settimeout(1) # set timeout for readline to avoid blocking
        handle(socket)
        socket.close()
...
def ok(socket, code, headers, *args):
    ...
    if len(args)==1:
        ...
        raise TypeError("ok() takes 4 or 5 positional arguments but "+ str(len(args)+3) +" were given")
